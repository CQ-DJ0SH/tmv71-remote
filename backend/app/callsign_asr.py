...
class CallsignRecognizer:
    ...
    def __init__(self, model_dir: str, own_call: str = "", min_conf: float = 0.55,
        ...
        self.last_text = ""                   # raw words of the last utterance (log)
        self.last_nbest: list = []            # last N-best callsign candidates (log)
    ...
    def _extract(self, result: dict) -> List[Tuple[str, float]]:
        ...
        if not alts:                              # no n-best (safety) -> single hyp
            alts = [result]
        # raw words of the top hypothesis (what Vosk actually heard) — for the log
        self.last_text = (alts[0].get("text") or "").strip()
        ...
        if not pool:
            self.last_nbest = []
            return []
        # rank all candidates the same way the winner is chosen (best first)
        ranked = sorted(pool.items(),
                        key=lambda kv: (kv[1][2], kv[1][0], -kv[1][1]), reverse=True)
        self.last_nbest = [c for c, _ in ranked]
        call, (conf, _rank, _valid) = ranked[0]
        return [(call, conf)]
