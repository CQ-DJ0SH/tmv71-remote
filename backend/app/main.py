...
class CallsignService:
    ...
    def _s_label(s: float) -> str:
        """Format the FM-quieting S value as S0..S9. The meter saturates at S9,
        so the +20/+40/+60 branch is only a guard for a future extension."""
        return "S%d" % int(round(s)) if s <= 9 else "+%d" % int(round(20 * (s - 9) / 2))

    def _rx_context(self) -> tuple:
        """(S-value label, band+freq label) at detection time, for the log."""
        s = ""
        try:
            s = self._s_label(self.audio.rx_s)
        except Exception:  # noqa: BLE001
            pass
        band = ""
        try:
            st = service.status
            ab = st.audio_band
            freq = next((b.rx_freq for b in (st.bands or []) if b.band == ab), None)
            band = ("A" if ab == 0 else "B") + (" %.4f" % (freq / 1e6) if freq else "")
        except Exception:  # noqa: BLE001
            pass
        return s, band
        ...
        while True:
            ...
            try:
                ...
                if (active or tail > 0) and pcm is not None and len(pcm):
                    calls = self._snap(await asyncio.to_thread(self._rec.feed, pcm))
                if active:
                    tail = 2
                elif tail > 0:
                    tail -= 1
                    if tail == 0:                    # tail fed -> finalise
                        calls += self._snap(await asyncio.to_thread(self._rec.flush))
            except Exception:  # noqa: BLE001
                calls = []
    ...
    def _snap(self, calls):
        ...
        feed() and flush() can both run in one loop iteration; the second one
        overwrites last_text/last_nbest, so reading them later in _emit would
        label a call with another segment's words."""
        text = self._rec.last_text if self._rec else ""
        nbest = list(self._rec.last_nbest[1:]) if self._rec else []
        return [(c, cf, text, nbest) for c, cf in calls]

    async def _emit(self, call: str, conf: float,
                    text: str = "", nbest: list | None = None) -> None:
        ...
        corr = f"  ⟵ {heard}" if call != heard else ""     # note any correction
        # extra debug context: raw words heard, other N-best candidates, S-value, band
